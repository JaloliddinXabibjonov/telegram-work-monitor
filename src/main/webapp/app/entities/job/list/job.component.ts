...
export class JobComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  private registerChangeList(): void {
    this.modifySubscription = this.eventManager.subscribe('jobListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IJob[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    ...
    if (this.itemsPerPage !== this.jobs.length) {
      window.scrollTo(0, 0);
    }
  }
